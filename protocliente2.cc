...
void atualizasensores(TPlaca placa){	
		placa.show();
		vector<TPsensor> auxS;
		placa.get_datapoints(auxS);
		int t =auxS.size();
		printf("Lendo %d sensores...Atualizando...\n", t);		
		if(t>0){
				for(int i = 0; i < t; i++){
					auxS[i].set_valor(50+i);
				  cout << "Valor do sensor [" << i << "] = " << auxS[i].get_valor() << endl;
			   }
			   placa.set_datapoints(auxS);		
		}		
}
...
//int main(){	
	...
	printf("Timer iniciado: %ld \n\n", mytimer);
  	...
  	TPsensor sensor[N];    	
   	for(int x=0; x<N; x++){
   		sensor[x].set_idSensor(x);
   		sensor[x].set_tipo(x+10);
  	...
  	atualizasensores(placa);
  	placa.show();
  	
  	/*ASN1Sequence<TPsensor> * psensor;  	
  	vector<TPsensor> s2;
  	placa.get_datapoints(s2);
  	cout << "\nMostrando s2................. " ;
  	...
  	sensor1.set_idSensor(2);
  	sensor1.set_tipo(22);
  	sensor1.set_pEnvio(222);
  	sensor1.set_pAmostragem(2222);
  	sensor1.set_valormax(22);
  	sensor1.set_valormin(2);
	...
	//&v1->idSensor = &sensor1->idSensor;
	//v1->set_idSensor(sensor1.get_idSensor());
	//v1->set_registros(sensor1.regs);
	//v2.set_idSensor(sensor2.get_idSensor());
	//v2.set_registros(&sensor1.get_regs());
